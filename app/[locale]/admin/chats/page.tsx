...
export default function AdminChatsPage() {
  ...
  // refresh list cada vez que llega nuevo mensaje (via store)
  useChatStore.getState(); // hint para habilitar reactividad
  ...
  return (
    <div className='-mx-4 sm:-mx-6 lg:-mx-8 -my-8'>
      <div className='h-[calc(100vh-10rem)] grid grid-cols-12 overflow-hidden bg-white dark:bg-background border border-border rounded-none sm:rounded-xl mx-0 sm:mx-6 lg:mx-8'>
        {/* Sidebar */}
        <aside className='col-span-12 md:col-span-4 lg:col-span-3 border-r border-border flex flex-col bg-white dark:bg-card'>
          ...
          <div className='px-4 py-3 space-y-2 border-b border-border'>
            ...
            <div className='flex gap-2'>
              ...
              <select
                value={statusFilter}
                onChange={(e) =>
                  setStatusFilter(e.target.value as ChatRoomStatus | '')
                }
                className='px-3 py-2 text-sm rounded-lg border border-border bg-background focus:outline-none focus:ring-2 focus:ring-[#f9bbc4]'
                aria-label='Estado'
              >
        ...
        {/* Panel */}
        <section className='col-span-12 md:col-span-8 lg:col-span-9 flex flex-col min-h-0 bg-[#fafafa] dark:bg-background'>
          {activeRoom ? (
            <ChatRoomPanel
              key={activeRoom.id}
              room={activeRoom}
            ...
            <div className='flex-1 flex flex-col items-center justify-center text-center px-8 text-muted-foreground'>
              ...
              <p className='text-sm max-w-sm'>
                Seleccioná una conversación de la izquierda para ver los
                mensajes.
              </p>
            </div>
          )}
...
function RoomListItem({
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left flex gap-3 px-4 py-3 border-b border-border/60 transition-colors ${
        active ? 'bg-[#f9bbc4]/20' : 'hover:bg-muted/40'
      }`}
    >
      <div className='w-10 h-10 rounded-full bg-[#f9bbc4] text-white flex items-center justify-center text-sm font-semibold flex-shrink-0'>
        {initials || '?'}
      </div>
      <div className='min-w-0 flex-1'>
        ...
        <div className='flex items-center justify-between gap-2 mt-0.5'>
          ...
          {(room.unread ?? 0) > 0 && (
            <span className='text-[10px] bg-[#f9bbc4] text-white rounded-full px-2 py-0.5 flex-shrink-0'>
              {room.unread}
            </span>
          )}
        </div>
