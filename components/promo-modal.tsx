...
import { PROMO_CONFIG, getTimeUntilPromoChange, isPromoDisabled } from '@/lib/promo-config';
...
export default function PromoModal() {
  ...
  useEffect(() => {
    ...
    const isHome = pathname === '/' || pathname === '/es' || pathname === '/en' || pathname?.match(/^\/[a-z]{2}$/);
    ...
    if (checkShouldShow()) {
      ...
      const timeout = setTimeout(() => {
        setIsOpen(true);
        localStorage.setItem('promo-modal-last-shown', DateTime.now().setZone('America/Argentina/Buenos_Aires').toMillis().toString());
  ...
  useEffect(() => {
    ...
    const updateTimer = () => {
      ...
      const diff = target.diff(now, ['days', 'hours', 'minutes', 'seconds', 'milliseconds']);
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
        <div
          className="rounded-2xl max-w-lg w-full p-8 relative pointer-events-auto"
        ...
        >
          ...
          <div className="text-center space-y-6">
            ...
            <div className="inline-flex items-center justify-center">
              <div className="bg-[#67111c] text-white px-6 py-2 rounded-full text-sm font-bold tracking-wider">
                ¡OFERTA ESPECIAL!
              </div>
            </div>
            ...
            <div className="space-y-2">
              <h2 className="text-4xl font-bold text-[#67111c]">
            ...
            <div className="space-y-2">
              <p className="text-base text-[var(--mg-dark)]">
                Cerra el año con una oportunidad de llevar tu profesión a otro nivel.
              </p>
              <p className="text-sm text-[var(--mg-gray)]">
                <strong className="font-bold text-[var(--mg-dark)]">Accesibles desde cualquier dispositivo</strong>
              </p>
              ...
              {isCountdownZero ? (
                ...
                <div className="space-y-3">
                  <p className="text-sm font-medium text-[var(--mg-dark)]">
                    {isPromoActive ? PROMO_CONFIG.TEXTS.countdownActive : PROMO_CONFIG.TEXTS.countdownBeforeStart}
                  </p>
                  <div className="flex justify-center gap-2">
                    {[
                      ...
                      <div key={idx} className="flex flex-col items-center">
                        <div 
                          className="text-[#67111c] rounded-lg px-3 py-2 min-w-[60px] font-mono text-2xl font-bold"
                          style={{ backgroundColor: '#fbe8ea', boxShadow: 'none' }}
                        >
                          {item.value}
                        </div>
